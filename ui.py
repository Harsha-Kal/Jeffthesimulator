...
def pending_velocity_from_mouse(px, py, mx, my):
    dx = float(mx - px)
    dy = float(my - py)
    d = math.hypot(dx, dy)
    if d < VEL_MOUSE_DEAD:
    ...
    t = min(1.0, (d - VEL_MOUSE_DEAD) / span)
    speed = t * VEL_SPEED_MAX
    vx, vy = ux * speed, uy * speed
    arrow_len = t * ARROW_LEN_MAX
...
class DragMenu:
    ...
    def _toggle_pending_freeze(self, px, py, mx, my):
        if self.pending.get("frozen"):
        ...
        if _mouse_over_pending_confirm_ui(px, py, mx, my):
            ...
            vx, vy, ux, uy, alen = pending_velocity_from_mouse(px, py, mx, my)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if self.pending:
                ...
                if self._in_game(mx, my) and not item.get("is_static"):
                    self._toggle_pending_freeze(px, py, mx, my)
                    return None, None
    ...
    def draw(self, screen, bodies):
        ...
        if display_item:
            ...
            if self.pending and not display_item.get("is_static"):
                lock = "locked" if self.pending.get("frozen") else "click space to lock aim"
                sub = f"Mass {display_item['mass']} • {lock}"
            else:
                sub = f"Mass {display_item['mass']}"
